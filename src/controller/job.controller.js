...
const addJobController = catchAsync(async (req, res) => {
  ...
  if (req?.user?.user?.role != "company") {
    ...
    const { companymessage, position, experience, location, availability } =
      req.body;
    if (
      !companymessage ||
      !position ||
      ...
      const jobAlreadyExist = await Job.findOne({
        companyId,
        position,
        experience,
        availability,
      ...
      res
        .status(200)
        .json({ data: jobDetails, message: "Job added succesfully" });
    }
  }
});
...
const fetchCompanyJobsController = catchAsync(async (req, res) => {
  if (req?.user?.user.role != "company") {
    ...
    const jobs = await Job.find({ companyId }).skip(skip).limit(limit);
...
const updateJobDetailController = catchAsync(async (req, res) => {
  if (req?.user?.user.role != "company") {
    ...
    const { id, companymessage, position, experience, location, availability } =
      req.body;
...
const deleteJobDetailsController = catchAsync(async (req, res) => {
  ...
  if (!studentDetailExist) {
    ...
    res
      .status(200)
      .json({ data: studentDetailExist, message: "deleted succesfully" });
  }
});
...
module.exports = {
  deleteJobDetailsAdminController,
  fetchJobsAdminController,
  addJobController,
  fetchJobsController,
  fetchCompanyJobsController,
  updateJobDetailController,
  deleteJobDetailsController,
};
