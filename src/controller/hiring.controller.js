...
const hiredStudentsController = catchAsync(async (req, res) => {
  ...
  if (req?.user?.user?.role != "company") {
    ...
    const hiredStudents = await HiredStudents.find({ companyId })
      .skip(skip)
      .limit(limit);
    ...
    res
      ...
      .json({
        data: hiredStudents,
        message: "Hired students fetched succesfully",
        pages,
      });
  }
});
const rejectHiringController = catchAsync(async (req, res) => {
  if (req?.user?.user?.role != "company") {
    ...
    const application = await Application.findOne({
      studentId: hiredOne.studentId,
      position: hiredOne.position,
    });
...
const hireStudentController = catchAsync(async (req, res) => {
  if (req?.user?.user?.role != "company") {
    ...
    const companyId = req?.user?.user?._id;
    const hiringExist = await HiredStudents.findOne({
      studentId,
      position,
      companyId,
    });
    ...
    const newHiring = new HiredStudents({
      email,
...
module.exports = {
  rejectHiringController,
  hiredStudentsController,
  hireStudentController,
};
